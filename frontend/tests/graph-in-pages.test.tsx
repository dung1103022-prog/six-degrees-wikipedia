...
vi.mock("../src/components/GraphView", async () => {
  ...
  return {
    default: ({ response }: { response: { levels: unknown[]; found: boolean } }) =>
      createElement("div", { "data-testid": "graph-view", "data-levels": String(response.levels.length), "data-found": String(response.found) }),
  };
});
...
async function search() {
  ...
  render(<App />);
  await user.type(screen.getByLabelText("Từ"), "S");
  await user.type(screen.getByLabelText("Đến"), "T");
...
describe("the graph in the pages", () => {
  ...
  it("search page: no graph before a result, and none after an error", async () => {
